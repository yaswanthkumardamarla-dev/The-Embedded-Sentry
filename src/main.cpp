/* Team Members
    * 1. Bhagya Rekha Deenadayal (bd2585)
    * 2. Yaswanth Kumar Damarla  (yd3034)
    * 3. Hemanth Sree Meka       (hm3324)
 */
...
const int button1_x = 50;
const int button1_y = 70;
const int button1_width = 145;
const int button1_height = 70;
const char *button1_label = "RECORD";
const int button2_x = 50;
const int button2_y = 170;
const int button2_width = 145;
const int button2_height = 70;
const char *button2_label = "UNLOCK";
const int message_x = 5;
const int message_y = 30;
const char *message = "PATTERN LOCK";
const int text_x = 5;
...
int main()
{
    lcd.Clear(LCD_COLOR_WHITE);   // Set the background to light blue using the hexadecimal value
...
void gyroscope_thread()
{
    ...
    while (1)
    {
        ...
        if (flag_check & ERASE_FLAG)
        {
            ...
            lcd.SetTextColor(LCD_COLOR_ORANGE);                   // Reset the text color
        ...
        if (flag_check & (KEY_FLAG | UNLOCK_FLAG))
        {
            ...
            lcd.SetTextColor(LCD_COLOR_WHITE);                  // Set the color to the background color
            lcd.FillRect(0, text_y, lcd.GetXSize(), FONT_SIZE); // Clear a specific line
            lcd.SetTextColor(LCD_COLOR_GREEN);                   // Reset the text color
            lcd.DisplayStringAt(text_x, text_y, (uint8_t *)display_buffer, CENTER_MODE);
        }
        ...
        else if (flag_check & UNLOCK_FLAG)
        {
            ...
            else // compare the unlocking record with the gesture key
            {
                ...
                else
                {
                    ...
                    lcd.SetTextColor(LCD_COLOR_WHITE);                  // Set the color to the background color
                    lcd.FillRect(0, text_y, lcd.GetXSize(), FONT_SIZE); // Clear a specific line
                    lcd.SetTextColor(LCD_COLOR_RED);                   // Reset the text color
                    lcd.DisplayStringAt(text_x, text_y, (uint8_t *)display_buffer, CENTER_MODE);
